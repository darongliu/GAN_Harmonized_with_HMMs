...
def compare_boundaries(predicts, reals, one2one=True, T=2):
    ...
    NC = hits.float()
    NT = predicts_loc.sum(1).float()
    NG = reals_loc.sum(1).float()
    return [NC, NT, NG]
...
def compute_F(tensor):
    ...
    RCL = NC / NG
    F = (2 * PRC * RCL) / (PRC + RCL)
    return [F.numpy(), PRC.numpy(), RCL.numpy()]
...
if __name__ == '__main__':
    ...
    uns1 = read_pickle(args.uns1_boundary_path)
    uns2 = read_pickle(args.uns2_boundary_path)
    uns3 = read_pickle(args.uns3_boundary_path)

    stats1 = compare_boundaries(uns1, orc)
    R1 = compute_R(stats1)
    stats2 = compare_boundaries(uns2, orc)
    R2 = compute_R(stats2)
    stats3 = compare_boundaries(uns3, orc)
    R3 = compute_R(stats3)
    ...
    # filter and determine 'value' for ranking
    for i in range(len(orc)):
        if not (R3[i] > R2[i] and R2[i] > R1[i]):
            continue

        index.append(i)
        value.append(R3[i]-R1[i])
    print('valid len: ', len(index))
        
    # ranking
    zipped_lists = zip(value, index)
    sorted_zipped_lists = sorted(zipped_lists, reverse=True)
    sorted_index = [index for _, index in sorted_zipped_lists]
    
    # print info
    for i in range(20):
        index = sorted_index[i]
        print(f'index: {index}, R1: {R1[index]}, R2: {R2[index]}, R3: {R3[index]}')
