...
word_symtab=$lang_or_graph/words.txt
phn_symtab=$lang_or_graph/phones.txt

for f in $word_symtab $phn_symtab $dir/lat.1.gz $data/text; do
...
if [ $stage -le 0 ]; then
  ...
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    ...
    if $decode_mbr ; then
      echo "not implementing; exiting ..." && exit 1;
	...
	ali-to-phones $model ark:- ark,t:- \| \
        utils/int2sym.pl -f 2- $phn_symtab \| \
        $hyp_filtering_cmd '>' $dir/scoring_kaldi/penalty_$wip/LMWT.txt || exit 1;
    fi
